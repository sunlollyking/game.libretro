...
void SAFE_DELETE_GAME_INFO(std::vector<CGameInfoLoader *> &vec) {
  for (std::vector<CGameInfoLoader *>::iterator it = vec.begin();
       it != vec.end(); ++it)
    delete *it;
  vec.clear();
}
...
ADDON_STATUS CGameLibRetro::Create() {
  try {
    ...
    CLibretroEnvironment::Get().InitializeEnvironment(this, &m_client,
                                                      &m_clientBridge);
    ...
    std::set<std::string>
        coreExtensions; // TODO: Parse string from libretro API
    std::set<std::string>
        addonExtensions; // TODO: Convert char** to set<string>

    if (coreExtensions != addonExtensions) {
      std::string strAddonExtensions; // = StringUtils::Join(addonExtensions,
                                      // "|"); // TODO
      esyslog("CORE: Extensions don't match addon.xml: %s",
              strAddonExtensions.c_str());
      throw ADDON_STATUS_PERMANENT_FAILURE;
    }

    if (SupportsVFS() != m_supportsVFS) {
      esyslog("CORE: VFS support doesn't match addon.xml: %s",
              SupportsVFS() ? "true" : "false");
      throw ADDON_STATUS_PERMANENT_FAILURE;
    }
...
ADDON_STATUS
CGameLibRetro::SetSetting(const std::string &settingName,
...
GAME_ERROR
CGameLibRetro::LoadGameSpecial(SPECIAL_GAME_TYPE type,
  ...
  bool result = m_client.retro_load_game_special(type, infoVec.data(),
  urls.size());
...
GAME_REGION CGameLibRetro::GetRegion() {
  return m_client.retro_get_region() == RETRO_REGION_NTSC ? GAME_REGION_NTSC
                                                          : GAME_REGION_PAL;
}
...
GAME_ERROR CGameLibRetro::RunFrameSpeculative() {
  // The frontend is about to rewind past this frame, so run the core and
  // nothing else. The achievement runtime is the reason this exists: driven by
  // a frame that gets taken back, it announces challenges starting and ending
  // and unlocks achievements for a frame that never really happened, and those
  // announcements have already left by the time the state is restored.
...
bool CGameLibRetro::HasFeature(const std::string &controller_id,
                               const std::string &feature_name) {
  return CButtonMapper::Get().GetLibretroIndex(controller_id, feature_name) >=
         0;
}
...
bool CGameLibRetro::ConnectController(bool connect,
  ...
  if (port < 0) {
    if (!CInputManager::Get().IsKeyboard(strPortAddress) &&
        !CInputManager::Get().IsMouse(strPortAddress))
      esyslog("Failed to connect controller, invalid port address: %s",
              strPortAddress.c_str());
  } else {
    libretro_device_t device = RETRO_DEVICE_NONE;

    if (connect) {
      device =
          CInputManager::Get().ConnectController(strPortAddress, controller_id);
    } else {
    ...
    if (CInputManager::Get().GetConnectionPortIndex(strPortAddress,
                                                    connectionPort))
      port = connectionPort;

    if (port >= 0) {
      dsyslog("Setting port \"%s\" (libretro port %d) to controller \"%s\" "
              "(libretro device ID %u)",
              strPortAddress.c_str(), port, strController.c_str(), device);

      m_client.retro_set_controller_port_device(port, device);
    } else {
      dsyslog(
          "Ignoring port \"%s\" with controller \"%s\" (libretro device ID %u)",
...
GAME_ERROR CGameLibRetro::Deserialize(const uint8_t *data, size_t size) {
  ...
  if (!m_client.retro_unserialize(data, size))
    return GAME_ERROR_FAILED;
...
GAME_ERROR CGameLibRetro::DeserializeAchievements(const uint8_t *data,
                                                  size_t size) {
  if (data == nullptr || size == 0)
    return GAME_ERROR_INVALID_PARAMETERS;
...
GAME_ERROR CGameLibRetro::GetMemory(GAME_MEMORY type, uint8_t *&data,
                                    size_t &size) {
  data = static_cast<uint8_t *>(m_client.retro_get_memory_data(type));
  size = m_client.retro_get_memory_size(type);
...
GAME_ERROR CGameLibRetro::SetCheat(unsigned int index, bool enabled,
...
GAME_ERROR CGameLibRetro::RCGetGameIDUrl(std::string &url,
...
GAME_ERROR
CGameLibRetro::SetRetroAchievementsCredentials(const std::string &username,
                                               const std::string &token) {
  CCheevos::Get().SetCredentials(username, token);
  return GAME_ERROR_NO_ERROR;
}

GAME_ERROR CGameLibRetro::RCPostRichPresenceUrl(
    std::string &url, std::string &postData, const std::string &username,
    const std::string &token, unsigned int gameID,
...
GAME_ERROR
CGameLibRetro::ActivateAchievement(unsigned cheevo_id,
                                   const std::string &memAddrExpression) {
  return GAME_ERROR_NOT_IMPLEMENTED;
}

GAME_ERROR CGameLibRetro::GetCheevoUrlId(
    const std::function<void(const std::string &achievementUrl,
...
bool CGameLibRetro::GetEjectState() { return m_clientBridge.GetEjectState(); }
...
GAME_ERROR CGameLibRetro::ReplaceImageIndex(unsigned int imageIndex,
                                            const std::string &filePath) {
...
GAME_ERROR CGameLibRetro::SetInitialImage(unsigned int imageIndex,
                                          const std::string &filePath) {
