...
unsigned int ParseHttpStatus(const std::string &responseProtocol) {
  ...
  return static_cast<unsigned int>(
      std::strtoul(responseProtocol.c_str() + start + 1, nullptr, 10));
}
} // namespace
...
void CCheevos::Initialize(kodi::addon::CInstanceGame *gameInstance,
...

  {
    ...
    std::string userAgent =
        std::string(RA_CLIENT_NAME) + "/" + GAME_LIBRETRO_VERSION;
    ...
    kodi::Log(ADDON_LOG_DEBUG, "CCheevos: user agent '%s'",
              m_userAgent.c_str());
  }
  ...
  rc_client_enable_logging(m_rcClient, RC_CLIENT_LOG_LEVEL_VERBOSE,
                           [](const char *message, const rc_client_t *) {
                             kodi::Log(ADDON_LOG_DEBUG, "rc_client: %s",
                                       message);
                           });
...
void CCheevos::BeginLogin() {
  ...
  if (userName.empty() || loginToken.empty()) {
    kodi::Log(ADDON_LOG_INFO,
              "CCheevos: no RA credentials yet, deferring login");
    return;
  }
  ...
  rc_client_begin_login_with_token(m_rcClient, userName.c_str(),
                                   loginToken.c_str(), RcheevosLoginCallback,
                                   this);
}
...
void CCheevos::SetCredentials(const std::string &username,
...
void CCheevos::SetHardcoreEnabled(bool enabled) {
  ...
  kodi::Log(ADDON_LOG_INFO, "CCheevos: hardcore mode %s",
...
void CCheevos::SetEncoreModeEnabled(bool enabled) {
  ...
  kodi::Log(ADDON_LOG_INFO, "CCheevos: encore mode %s",
            enabled ? "enabled" : "disabled");
}
  ...
  if (signature != m_lastProgressSignature) {
    ...
    if (!progress.empty()) {
      kodi::Log(ADDON_LOG_DEBUG,
                "CCheevos: publishing progress for %zu achievement(s): %s",
  ...
  if (required > size) {
    ...
    if (!m_progressTooLargeWarned) {
      ...
      kodi::Log(ADDON_LOG_WARNING,
                "CCheevos: achievement progress needs %zu bytes but only %zu "
                "are reserved in the "
...
bool CCheevos::DeserializeProgress(const uint8_t *buffer, size_t size) {
  ...
  return rc_client_deserialize_progress_sized(m_rcClient, buffer, size) ==
         RC_OK;
}
...
void CCheevos::RcheevosServerCall(const rc_api_request_t *request,
  ...
  const std::string postData =
      (request->post_data != nullptr) ? request->post_data : "";
  ...
  auto task = std::async(std::launch::async, [url, postData, userAgent,
  ...
  // memory. Deinitialize() waits for everything held here first.
  {
    ...
    std::erase_if(s_instance->m_serverCalls, [](const std::future<void> &call) {
      return call.wait_for(std::chrono::seconds(0)) ==
...
uint32_t CCheevos::RcheevosReadMemory(uint32_t address, uint8_t *buffer,
  ...
  if (!s_instance->m_memoryInitialized) {
    ...
    if (!s_instance->m_memoryInitialized) {
      ...
      retroMemoryMap.num_descriptors =
      ...
      kodi::Log(ADDON_LOG_INFO,
                "CCheevos: memory mapped for console %u from %zu descriptors, "
                "total_size=%u",
  ...
  return rc_libretro_memory_read(&s_instance->m_memoryRegions, address, buffer,
                                 num_bytes);
}
...
// Event handler
void CCheevos::RcheevosEventHandler(const rc_client_event_t *event,
                                    rc_client_t *client) {
  ...
  case RC_CLIENT_EVENT_SERVER_ERROR: {
    ...
    const char *message = (err != nullptr && err->error_message != nullptr)
                              ? err->error_message
                              : "Unknown error";
    const char *api = (err != nullptr && err->api != nullptr) ? err->api : "";

    kodi::Log(ADDON_LOG_WARNING,
              "CCheevos: server error from RetroAchievements: %s", message);
    s_instance->m_gameInstance->KodiRCOnServerError(message, api);
    break;
  }
  case RC_CLIENT_EVENT_DISCONNECTED: {
    kodi::Log(ADDON_LOG_WARNING,
              "CCheevos: disconnected from RetroAchievements");
  ...
  case RC_CLIENT_EVENT_SUBSET_COMPLETED: {
    ...
    const char *title =
        (subset != nullptr && subset->title != nullptr) ? subset->title : "";
  ...
  case RC_CLIENT_EVENT_LEADERBOARD_SUBMITTED: {
    ...
    if (lb != nullptr) {
      ...
      if (event->type == RC_CLIENT_EVENT_LEADERBOARD_STARTED)
        s_instance->m_gameInstance->KodiRCOnLeaderboardStarted(data);
      else if (event->type == RC_CLIENT_EVENT_LEADERBOARD_FAILED)
        s_instance->m_gameInstance->KodiRCOnLeaderboardFailed(data);
      else
        s_instance->m_gameInstance->KodiRCOnLeaderboardSubmitted(data);
    }
    break;
  }
  case RC_CLIENT_EVENT_LEADERBOARD_SCOREBOARD: {
    const rc_client_leaderboard_scoreboard_t *sb =
    ...
    if (sb != nullptr) {
      ...
      kodi::Log(ADDON_LOG_INFO, "CCheevos: leaderboard %u placed %u of %u",
                sb->leaderboard_id, sb->new_rank, sb->num_entries);

      s_instance->m_gameInstance->KodiRCOnLeaderboardScoreboard(data);
    }
    break;
  }
  ...
  case RC_CLIENT_EVENT_LEADERBOARD_TRACKER_UPDATE: {
    ...
    if (tracker != nullptr) {
      ...
      if (event->type == RC_CLIENT_EVENT_LEADERBOARD_TRACKER_HIDE)
        s_instance->m_gameInstance->KodiRCOnLeaderboardTracker(data, false);
      else if (event->type == RC_CLIENT_EVENT_LEADERBOARD_TRACKER_UPDATE)
        s_instance->m_gameInstance->KodiRCOnLeaderboardTrackerUpdate(data);
      else
        s_instance->m_gameInstance->KodiRCOnLeaderboardTracker(data, true);
    }
    break;
  }
  default:
...
void CCheevos::RcheevosLoginCallback(int result, const char *errorMessage,
  ...
  if (user != nullptr) {
    {
      ...
      cheevos->m_userName =
          (user->username != nullptr) ? user->username : cheevos->m_userName;
    }
  ...
  if (!cheevos->m_gamePath.empty()) {
    kodi::Log(ADDON_LOG_INFO, "CCheevos: identifying game: %s",
              cheevos->m_gamePath.c_str());
    ...
    rc_client_begin_identify_and_load_game(
        client, RC_CONSOLE_UNKNOWN, cheevos->m_gamePath.c_str(), nullptr, 0,
...
void CCheevos::RcheevosGameLoadCallback(int result, const char *errorMessage,
  ...
  if (gameInfo != nullptr && gameInfo->title != nullptr &&
    ...
    kodi::Log(ADDON_LOG_INFO,
              "CCheevos: '%s' - this ROM version has no achievements",
              gameInfo->title);
  ...
  // which remains valid until rc_client_destroy_achievement_list() is called
  // below.
  ...
  if (achList != nullptr) {
    for (uint32_t b = 0; b < achList->num_buckets; ++b) {
      ...
      for (uint32_t a = 0; a < bucket.num_achievements; ++a) {
        ...
        if (ach->title != nullptr &&
            std::strncmp(ach->title, "Warning:", 8) == 0) {
          kodi::Log(ADDON_LOG_INFO, "CCheevos: skipping system notice '%s'",
                    ach->title);
          continue;
        }
  ...
  cheevos->m_richPresenceThread =
      std::thread(&CCheevos::RichPresencePingThread, cheevos);
}
...
void CCheevos::RichPresencePingThread() {
  while (m_richPresenceRunning) {
    ...
    for (int i = 0; i < (RP_PING_INTERVAL_MS / RP_SLEEP_INTERVAL_MS) &&
                    m_richPresenceRunning;
         ++i)
      std::this_thread::sleep_for(
          std::chrono::milliseconds(RP_SLEEP_INTERVAL_MS));
  }
}
